...
class ProfileHeaderView: UIView {
    ...
    private lazy var SetTextView: UITextField = {
        ...
        // В TextField  нельзя сразу класть текст
        statusTextField.placeholder = "aaaaaas"
    ...
    private func  setupView(){
        ...
        NSLayoutConstraint.activate([
            ...
            self.headerLabelView.topAnchor.constraint(equalTo: self.safeAreaLayoutGuide.topAnchor, constant: 27),
            ...
            self.SetTextView.leadingAnchor.constraint(equalTo: avatarImageView.trailingAnchor, constant: 10),
    ...
    @objc func statusTextChanged(_ textField: UITextField){
        if SetTextView.hasText {
            statusText = textField.text ?? "empty"
            print("Переменная: \(statusText)")
        } else {
            print ("Пусто")
            // Сброисть последний символ в  записавшегося из Text Field
            statusText = nil
